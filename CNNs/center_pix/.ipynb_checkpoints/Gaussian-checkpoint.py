...
def generate_model(kernel_size, pool_size, activation, strides, input_shape,im_size=128):
    ...
    model.add(layers.Conv2D(filters=16, kernel_size=kernel_size, activation=activation))
    ...
    model.add(layers.Conv2D(filters=32, kernel_size=kernel_size, activation=activation))

    # 4. 2×2, stride-2 max pooling
    model.add(layers.MaxPooling2D(pool_size=pool_size, strides=strides))

    # 5. 3×3 convolution with 64 filters
    model.add(layers.Conv2D(filters=64, kernel_size=kernel_size, activation=activation))
...
def plot_1tot1(y_train,y_train_model,spath):
    ...
    plt.legend()
    plt.tight_layout()    
    plt.savefig(spath + '/1to1_center_xy.png', dpi=200, bbox_inches='tight')
    print("\n---> 1to1 plot saved to:", spath)

    #plt.show()
    plt.close()
    
def plot_metrics(history,spath):
    ...
    plt.savefig(spath + '/accuracy_loss.png',dpi=200, bbox_inches='tight')
    #plt.show()
...
def main():
    ...
    if mode == 'cpu':
        home = '/Users/erebor/repos/neural/CNNs/center_pix'
    ...
    dataset = create_set(im_size=im_size)
    ...
    print(model_dir)
    os.mkdir(model_dir)
    os.mkdir(model_dir + '/assets')
    os.mkdir(model_dir + '/figs')

    model.save(model_dir + '/assets')

    print("Model assets saved to:", model_dir + '/assets')
    # plot loss and accuracy
    plot_metrics(history=history,spath=model_dir)
